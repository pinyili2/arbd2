...
// ============================================================================
// Test Fixture for CUDA Kernel Tests
// ============================================================================
struct CUDAKernelTestFixture {
	...
	CUDAKernelTestFixture() {
		try {
			...
			if (CUDA::CUDAManager::all_devices().empty()) {
				WARN("No CUDA devices found. Skipping CUDA kernel tests.");
				return;
			}
...
TEST_CASE_METHOD(CUDAKernelTestFixture,
				 "CUDA Direct Backend Functions",
				 "[CUDA][kernels][backend]") {
	if (!CUDA_available) {
		SKIP("CUDA not available");
	}

	SECTION("CUDA kernel launch function") {
		const size_t n = 256;
		...
		auto event = launch_cuda_kernel(CUDA_resource, n, inputs, outputs, config, scale_kernel);
		event.wait();
...
TEST_CASE_METHOD(CUDAKernelTestFixture, "CUDA Utility Functors", "[CUDA][kernels][utils]") {
	if (!CUDA_available) {
		SKIP("CUDA not available");
	}
	...
	SECTION("Copy functor test") {
		...
		KernelConfig config;
		auto event = launch_cuda_kernel(CUDA_resource, n, inputs, outputs, config, copy_func);
		event.wait();
	...
	SECTION("Fill functor test") {
		...
		auto event = launch_cuda_kernel(CUDA_resource, n, inputs, outputs, config, fill_func);
		event.wait();
...
TEST_CASE_METHOD(CUDAKernelTestFixture, "CUDA Configuration Testing", "[CUDA][kernels][config]") {
	if (!CUDA_available) {
		SKIP("CUDA not available");
	}
	...
	SECTION("Different block sizes") {
		...
		for (size_t block_size : block_sizes) {
			...
			auto event =
				launch_cuda_kernel(CUDA_resource, n, inputs, outputs, config, double_kernel);
			event.wait();
	...
	SECTION("Async vs sync execution") {
		...
		// Synchronous execution
		{
			...
			auto event = launch_cuda_kernel(CUDA_resource, n, inputs, outputs, config, add_kernel);
			event.wait();
		}
...
TEST_CASE_METHOD(CUDAKernelTestFixture, "CUDA Matrix Operations", "[CUDA][kernels][matrix]") {
	if (!CUDA_available) {
		SKIP("CUDA not available");
	}
	...
	SECTION("Matrix transpose") {
		...
		auto event =
			launch_cuda_kernel(CUDA_resource, total, inputs, outputs, config, transpose_kernel);
		event.wait();
...
TEST_CASE_METHOD(CUDAKernelTestFixture, "CUDA Error Handling", "[CUDA][kernels][errors]") {
	if (!CUDA_available) {
		SKIP("CUDA not available");
	}
	...
	SECTION("Large configuration values") {
		...
		for (const auto& block_config : oversized_blocks) {
			...
			REQUIRE_NOTHROW(
				launch_cuda_kernel(CUDA_resource, n, inputs, outputs, config, simple_kernel));
...
TEST_CASE_METHOD(CUDAKernelTestFixture,
	...
	SECTION("Throughput test with different data sizes") {
		...
		for (size_t n : sizes) {
			...
			auto event =
				launch_cuda_kernel(CUDA_resource, n, inputs, outputs, config, scale_kernel);
			event.wait();
