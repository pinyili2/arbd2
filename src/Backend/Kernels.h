...
struct KernelConfig {
	...
	size_t shared_memory = 0; // Shared memory size in bytes
...
Event launch_cuda_kernel(const Resource& resource,
	...
	dim3 grid(local_config.grid_size);
	dim3 block(local_config.block_size);
...
Event launch_sycl_kernel(const Resource& resource,
	...
	// Get pointers from buffer objects
	EventList deps;
	auto input_ptr = inputs.get_read_access(deps);
	auto output_ptr = outputs.get_write_access(deps);
...
void kernel_call_generic(const Resource& resource,
	...
	// Merge dependencies from accessors with config dependencies
	KernelConfig merged_config = config;
	for (const auto& event : depends_list.get_events()) {
		merged_config.dependencies.add(event);
	}

	// Backend dispatch - no type knowledge needed
	Event completion_event = dispatch_kernel(resource,
											 thread_count,
											 input_ptrs,
											 output_ptrs,
											 std::forward<Functor>(kernel_func),
...
Event launch_kernel_impl(const Resource& resource,
	...
	// Configure grid size if needed
	KernelConfig local_config = config;
	local_config.auto_configure(n);
	...
	if (resource.is_device()) {
		...
		cuda_kernel_wrapper<<<grid, block, local_config.shared_memory>>>(n, kernel, args...);
		...
		auto& device = SYCL::SYCLManager::get_current_device();
		auto& queue = device.get_next_queue();
		auto event = queue.get().parallel_for(sycl::range<1>(n),
											  [=](sycl::id<1> idx) { kernel(idx[0], args...); });
		if (!config.async) {
			event.wait();
		}
		return Event(event, resource);
...
class KernelChain {
  ...
  public:
	...
	KernelChain& then(size_t num_elements,
		...
		auto event = simple_kernel(resource_,
								   num_elements,
...
Event copy_async(const DeviceBuffer<T>& source,
	...
	return kernel_call(
		...
		source.size(),
		[](size_t i, const T* src, T* dst) { dst[i] = src[i]; },
		config,
		kernel_name);
}
...
struct ExecutionPolicy {
	enum class Type { SEQUENTIAL, PARALLEL, ASYNC };
