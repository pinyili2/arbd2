...
template <typename T, typename Enable = void> struct Proxy {
  ...
  RetType callSync(RetType (T::*memberFunc)(Args1...), Args2 &&...args) {
    ...
    case Resource::CUDA:
        result_ptr = ProxyImpl::cuda_call_sync_impl(
            addr, reinterpret_cast<void*>(&memberFunc), 
            &pack, sizeof(pack), location, sizeof(RetType));
        break;
    case Resource::SYCL:
        result_ptr = ProxyImpl::sycl_call_sync_impl(
            addr, reinterpret_cast<void*>(&memberFunc), 
            &pack, sizeof(pack), location, sizeof(RetType));
        break;
    case Resource::METAL:
        result_ptr = ProxyImpl::metal_call_sync_impl(
            addr, reinterpret_cast<void*>(&memberFunc), 
            &pack, sizeof(pack), location, sizeof(RetType));
        break;
    default:
  ...
  std::future<RetType> callAsync(RetType (T::*memberFunc)(Args1...), Args2 &&...args) {
    ...
    case Resource::CUDA:
        return std::async(std::launch::async, [this, pack, memberFunc]() {
            void* result_ptr = ProxyImpl::cuda_call_sync_impl(
                addr, reinterpret_cast<void*>(&memberFunc), 
                const_cast<ArgPack*>(&pack), sizeof(pack), location, sizeof(RetType));
    ...
    case Resource::SYCL:
        return std::async(std::launch::async, [this, pack, memberFunc]() {
            void* result_ptr = ProxyImpl::sycl_call_sync_impl(
                addr, reinterpret_cast<void*>(&memberFunc), 
                const_cast<ArgPack*>(&pack), sizeof(pack), location, sizeof(RetType));
    ...
    case Resource::METAL:
        return std::async(std::launch::async, [this, pack, memberFunc]() {
            void* result_ptr = ProxyImpl::metal_call_sync_impl(
                addr, reinterpret_cast<void*>(&memberFunc), 
                const_cast<ArgPack*>(&pack), sizeof(pack), location, sizeof(RetType));
...
HOST inline Proxy<T> _send_ignoring_children(const Resource &location, T &obj, T *dest = nullptr) {
    ...
    void* result_ptr = ProxyImpl::send_ignoring_children_impl(location, obj, dest);
...
Proxy<T> construct_remote(Resource location, Args &&...args) {
    ...
    void* result_ptr = ProxyImpl::construct_remote_impl<T>(location, &pack, sizeof(pack));
    T* typed_ptr = static_cast<T*>(result_ptr);
    
    // Create a temporary object for metadata (this is a limitation of the current design)
