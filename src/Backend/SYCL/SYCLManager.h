...
#define SYCL_CHECK(call) try { call; } catch (const sycl::exception& e) { check_sycl_error(e, __FILE__, __LINE__); }

/**
 * @brief Modern RAII wrapper for SYCL device memory
 * 
 * This class provides a safe and efficient way to manage SYCL device memory with RAII semantics.
...
public:
    ...
    DeviceMemory& operator=(DeviceMemory&& other) noexcept {
        if (this != &other) {
            if (ptr_ && queue_) sycl::free(ptr_, *queue_);
            ptr_ = std::exchange(other.ptr_, nullptr);
            size_ = std::exchange(other.size_, 0);
            queue_ = std::exchange(other.queue_, nullptr);
        }
    ...
    void copyFromHost(std::span<const T> host_data) {
        ...
        if (!ptr_ || host_data.empty() || !queue_) return;
...
public:
    ...
    [[nodiscard]] bool is_complete() const {
        if (!event_.has_value()) return true;
 ...
 * 
 * This class provides a comprehensive SYCL device management system with support for multiple devices,
...
public:
     ...
     * 
     * This nested class represents a single SYCL device and manages its resources,
    ...
    public:
        ...
        [[nodiscard]] size_t max_work_group_size() const noexcept { return max_work_group_size_; }
        [[nodiscard]] size_t max_compute_units() const noexcept { return max_compute_units_; }
        [[nodiscard]] size_t global_mem_size() const noexcept { return global_mem_size_; }
        [[nodiscard]] size_t local_mem_size() const noexcept { return local_mem_size_; }
        [[nodiscard]] bool is_cpu() const noexcept { return is_cpu_; }
        [[nodiscard]] bool is_gpu() const noexcept { return is_gpu_; }
        [[nodiscard]] bool is_accelerator() const noexcept { return is_accelerator_; }
