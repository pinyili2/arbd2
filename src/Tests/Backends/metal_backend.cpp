    ...
    SECTION("Device properties validation") {
        ...
        for (size_t i = 0; i < devices.size(); ++i) {
            ...
            LOGINFO("  Recommended max working set: {:.1f} MB",
                    static_cast<float>(device.recommended_max_working_set_size()) / (1024.0f * 1024.0f));
        }
    }
}
...
TEST_CASE("METALManager Command Queue Management", "[METALManager][Backend]") {
    ...
    SECTION("Command queue creation") {
        ...
        LOGINFO("Queue 1 address: {}, Queue 2 address: {}", 
                static_cast<void*>(&queue1), static_cast<void*>(&queue2));
        LOGINFO("Queue 1 MTL queue: {}, Queue 2 MTL queue: {}", 
                queue1.get(), queue2.get());
    }
    ...
    SECTION("Library access") {
        ...
        if (library != nullptr) {
            ...
            if (function_names && function_names->count() > 0) {
                ...
                for (NS::UInteger i = 0; i < std::min(function_names->count(), NS::UInteger(5)); ++i) {
        ...
        if (library != nullptr) {
            ...
            if (function_names && function_names->count() > 0) {
                ...
                if (first_name) {
                    ...
                    try {
                        auto* pipeline_state = ARBD::METAL::METALManager::get_compute_pipeline_state(func_name);
                        if (pipeline_state) {
                        ...
                        LOGINFO("Expected failure creating pipeline for {}: {}", func_name, e.what());
                    }
                }
            }
        }
