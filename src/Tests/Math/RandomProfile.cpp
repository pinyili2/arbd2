...
TEST_CASE_METHOD(ProfiledRandomTestFixture, "Random Generation Comparative Performance", "[random][profiling][comparison]") {
    ...
    SECTION("Uniform generation performance comparison") {
        constexpr size_t PERF_SIZE = 10000000; // 10M elements
        
        std::vector<std::pair<std::string, double>> performance_results;
        
        for (const auto& [resource, backend_name] : available_backends) {
            PROFILE_RANGE("Comparative::Uniform::" + backend_name, resource.type);
            
            Random<Resource> rng(resource, 256); // Larger state for performance
            rng.init(123456, 0);
            ...
            device_buffer.copy_to_host(sample.data(), 1000);
            
            auto [min_val, max_val] = std::minmax_element(sample.begin(), sample.end());
            REQUIRE(*min_val >= 0.0f);
            REQUIRE(*max_val <= 1.0f);
            
            LOGINFO("{} generated {} uniform numbers in {:.3f} ms ({:.1f} M numbers/sec)",
        ...
        LOGINFO("Fastest backend for uniform generation: {} ({:.3f} ms)", fastest->first, fastest->second);
        
        // Report relative performance
        for (const auto& [name, time] : performance_results) {
            double relative_speed = fastest->second / time;
            LOGINFO("{} relative speed: {:.2f}x", name, relative_speed);
        }
    }
