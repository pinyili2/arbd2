...
public:
    ...
    Event(const ARBD::METAL::Event& metal_event, const Resource& res) 
        : resource_(res), event_impl_(std::make_shared<ARBD::METAL::Event>(std::move(const_cast<ARBD::METAL::Event&>(metal_event)))) {}
#endif

    void wait() const {
        if (!event_impl_) return;
