...
void* cuda_call_sync_impl(void* addr, void* func_ptr, void* args, size_t args_size, 
...
std::future<void*> cuda_call_async_impl(void* addr, void* func_ptr, void* args, 
    ...
    if (location.is_local()) {
        return std::async(std::launch::async, [=] {
            return cuda_call_sync_impl(addr, func_ptr, args, args_size, location, result_size);
        });
    } else {
        return std::async(std::launch::async, [=] {
            int current_device;
            ...
            void* result = cuda_call_sync_impl(addr, func_ptr, args, args_size, location, result_size);
...
template<typename T>
void* cuda_construct_remote_impl(const Resource& location, void* args, size_t args_size) {
    if (location.is_local()) {
        T* devptr = nullptr;
...
template void* cuda_construct_remote_impl<int>(const Resource&, void*, size_t);
template void* cuda_construct_remote_impl<float>(const Resource&, void*, size_t);
template void* cuda_construct_remote_impl<double>(const Resource&, void*, size_t);

// Specialized CUDA send implementation (if needed)
template<typename T>
void* cuda_send_ignoring_children_impl(const Resource& location, T& obj, T* dest) {
    if (location.is_local()) {
        if (dest == nullptr) {
...
template void* cuda_send_ignoring_children_impl<int>(const Resource&, int&, int*);
template void* cuda_send_ignoring_children_impl<float>(const Resource&, float&, float*);
template void* cuda_send_ignoring_children_impl<double>(const Resource&, double&, double*);
...
template __global__ void proxy_sync_call_kernel<int, int>(int*, int*, int(int::*)(), );
template __global__ void proxy_sync_call_kernel<float, float>(float*, float*, float(float::*)(), );
