...
inline void check_nccl_error(ncclResult_t result, std::string_view file,
                             int line) {
  if (result != ncclSuccess) {
    ARBD_Exception(ExceptionType::CUDARuntimeError, "NCCL error at {}:{}: {}",
...
public:
  ...
  concept NCCLSupported =
      ...
      (std::is_same_v<T, float> || std::is_same_v<T, double> ||
       std::is_same_v<T, int> || std::is_same_v<T, long long> ||
  ...
  static void broadcast(int root, T **device_ptrs, size_t num_gpus,
                        size_t count, int stream_id = -1);

  // Convenience overload for std::vector
  template <NCCLSupported T>
  static void broadcast(int root, const std::vector<T *> &device_ptrs,
                        size_t count, int stream_id = -1) {
    broadcast(root, const_cast<T **>(device_ptrs.data()), device_ptrs.size(),
              count, stream_id);
  }
   ...
   */
  template <NCCLSupported T>
  static void allreduce(T **device_ptrs, size_t num_gpus, size_t count,
                        ncclRedOp_t op = ncclSum, int stream_id = -1);

  // Convenience overload for std::vector
  template <NCCLSupported T>
  static void allreduce(const std::vector<T *> &device_ptrs, size_t count,
                        ncclRedOp_t op = ncclSum, int stream_id = -1) {
    allreduce(const_cast<T **>(device_ptrs.data()), device_ptrs.size(), count,
              op, stream_id);
  }
   ...
   */
  template <NCCLSupported T>
  static void reduce(int root, const T **send_ptrs, T **recv_ptrs,
                     size_t num_gpus, size_t count, ncclRedOp_t op = ncclSum,
                     int stream_id = -1);
   ...
   */
  template <NCCLSupported T>
  static void allgather(const T **send_ptrs, T **recv_ptrs, size_t num_gpus,
                        size_t count, int stream_id = -1);
   ...
   */
  template <NCCLSupported T>
  static void scatter(int root, const T *send_ptr, T **recv_ptrs,
                      size_t num_gpus, size_t count, int stream_id = -1);
  ...
  static void nccl_broadcast(int root, const std::vector<T *> &send_d,
                             const std::vector<T *> &recv_d, size_t size,
                             int stream_id = -1) {
    broadcast<T>(root, const_cast<T **>(send_d.data()), send_d.size(), size,
                 stream_id);
  }
  ...
  static void nccl_reduce(int root, const std::vector<T *> &send_d,
                          const std::vector<T *> &recv_d, size_t size,
                          int stream_id = -1) {
    reduce<T>(root, const_cast<const T **>(send_d.data()),
              const_cast<T **>(recv_d.data()), send_d.size(), size, ncclSum,
              stream_id);
  }
   ...
   */
  template <NCCLSupported T>
  static void nccl_allreduce(const std::vector<T *> &send_d, size_t size,
                             int stream_id = -1) {
    allreduce<T>(const_cast<T **>(send_d.data()), send_d.size(), size, ncclSum,
                 stream_id);
  }
...
void NCCLManager::broadcast(int root, T **device_ptrs, size_t num_gpus,
  ...
  for (size_t i = 0; i < comms_.size(); ++i) {
    ...
    NCCL_CHECK(ncclBroadcast(static_cast<const void *>(device_ptrs[i]),
                             static_cast<void *>(device_ptrs[i]), count,
...
template <NCCLManager::NCCLSupported T>
void NCCLManager::allreduce(T **device_ptrs, size_t num_gpus, size_t count,
                            ncclRedOp_t op, int stream_id) {
  if (!is_initialized()) {
    ARBD_Exception(ExceptionType::ValueError, "NCCL not initialized");
  }
  ...
  for (size_t i = 0; i < comms_.size(); ++i) {
    ...
    NCCL_CHECK(ncclAllReduce(static_cast<const void *>(device_ptrs[i]),
                             static_cast<void *>(device_ptrs[i]), count,
                             get_nccl_type<T>(), op, comms_[i], stream));
  }
  NCCL_CHECK(ncclGroupEnd());
}

template <NCCLManager::NCCLSupported T>
void NCCLManager::reduce(int root, const T **send_ptrs, T **recv_ptrs,
                         size_t num_gpus, size_t count, ncclRedOp_t op,
                         int stream_id) {
  if (!is_initialized()) {
    ARBD_Exception(ExceptionType::ValueError, "NCCL not initialized");
  }
  ...
  for (size_t i = 0; i < comms_.size(); ++i) {
    ...
    NCCL_CHECK(ncclReduce(static_cast<const void *>(send_ptrs[i]),
                          static_cast<void *>(recv_ptrs[i]), count,
                          get_nccl_type<T>(), op, root, comms_[i], stream));
  }
  NCCL_CHECK(ncclGroupEnd());
}

template <NCCLManager::NCCLSupported T>
void NCCLManager::allgather(const T **send_ptrs, T **recv_ptrs, size_t num_gpus,
                            size_t count, int stream_id) {
  if (!is_initialized()) {
    ARBD_Exception(ExceptionType::ValueError, "NCCL not initialized");
  }
  ...
  for (size_t i = 0; i < comms_.size(); ++i) {
    ...
    NCCL_CHECK(ncclAllGather(static_cast<const void *>(send_ptrs[i]),
                             static_cast<void *>(recv_ptrs[i]), count,
                             get_nccl_type<T>(), comms_[i], stream));
  }
  NCCL_CHECK(ncclGroupEnd());
}

template <NCCLManager::NCCLSupported T>
void NCCLManager::scatter(int root, const T *send_ptr, T **recv_ptrs,
                          size_t num_gpus, size_t count, int stream_id) {
  if (!is_initialized()) {
    ARBD_Exception(ExceptionType::ValueError, "NCCL not initialized");
  }
  ...
  for (size_t target_rank = 0; target_rank < comms_.size(); ++target_rank) {
    ...
    for (size_t i = 0; i < comms_.size(); ++i) {
      ...
      const void *send_data =
          (i == target_rank) ? static_cast<const void *>(src) : nullptr;
      void *recv_data =
          (i == target_rank) ? static_cast<void *>(recv_ptrs[i]) : nullptr;
...
template <typename T> constexpr ncclDataType_t NCCLManager::get_nccl_type() {
