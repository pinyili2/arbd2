...
Patch Patch::send_children(Resource location) const {
	...
	case Resource::GPU:
		...
		//     For example, we could always move, or we could never move, or we could move if
		//     location metadata.data.location is a CPU resource
...
size_t Patch::send_particles_filtered(Proxy<Patch>& destination,
									  std::function<bool(size_t, Patch::Data)> filter) {
	// TODO determine who allocates and cleans up temporary data
	// TODO determine if there is a good way to avoid having temporary data (this can be done later
	// if delegated to a reasonable object)
	...
	// TODO currently everything is sychronous, but that will change; how do we avoid race
	// conditions? E.g. have a single array allocated with known start and end for each PE/Patch?
...
size_t Patch::Data::send_particles_filtered(Proxy<Patch::Data>& destination,
											std::function<bool(size_t, Patch::Data)> filter) {
	// Data buffer;		// not sure which object should allocate
