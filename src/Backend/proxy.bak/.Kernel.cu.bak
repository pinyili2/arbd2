...
Event launch_cuda_lambda(const Resource& resource,
    ...
    cudaStream_t stream = config.async ? 
        CUDAStreamManager::instance().get_next_stream() : 0;
    
    // Create wrapper
    CUDAKernelWrapper<Lambda, Args*...> wrapper{lambda};
    
    // Launch kernel
    cuda_generic_kernel<<<grid, block, config.shared_memory, stream>>>(
        n, wrapper, args...);
...
Event reduce_async_cuda(const DeviceBuffer<T>& input,
    ...
    DeviceBuffer<T> temp(grid_size, resource);
    ...
    cuda_reduce_kernel<<<grid_size, block_size, block_size * sizeof(T), stream>>>(
        input_ptr, temp_ptr, n, op);
    
    // Final reduction
    if (grid_size > 1) {
        cuda_reduce_kernel<<<1, grid_size, grid_size * sizeof(T), stream>>>(
            temp_ptr, output_ptr, grid_size, op);
    } else {
        cudaMemcpyAsync(output_ptr, temp_ptr, sizeof(T), 
...
template<typename T>
__global__ void cuda_matmul_kernel(const T* A, const T* B, T* C,
...
Event matmul_cuda(const DeviceBuffer<T>& A,
    ...
    cuda_matmul_kernel<<<grid, block, 0, stream>>>(
...
#define INSTANTIATE_CUDA_KERNELS(T) \
    ...
    template Event reduce_async_cuda<T, thrust::plus<T>>(const DeviceBuffer<T>&, DeviceBuffer<T>&, const Resource&, thrust::plus<T>); \
    template Event matmul_cuda<T>(const DeviceBuffer<T>&, const DeviceBuffer<T>&, DeviceBuffer<T>&, size_t, size_t, size_t, const Resource&);
