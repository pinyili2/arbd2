...
TEST_CASE("SYCLManager Basic Initialization", "[SYCLManager][Backend]") {
    SECTION("Initialize and discover devices") {
        ...
        for (size_t i = 0; i < all_devices.size(); ++i) {
            ...
            LOGINFO("Device {}: {} ({})", i, device.name(), device.vendor());
    ...
    SECTION("Unified shared memory") {
        ...
        try {
            ...
            if (usm_ptr != nullptr) {
                ...
                // Use in kernel
                queue.get().parallel_for(sycl::range<1>(test_size), [=](sycl::id<1> idx) {
                    usm_ptr[idx] *= 2.0f;
                }).wait();
...
TEST_CASE("SYCLManager Device Discovery Details", "[SYCLManager][Backend]") {
    ...
    SECTION("Device capabilities") {
        for (size_t i = 0; i < all_devices.size(); ++i) {
            ...
            LOGINFO("  Vendor: {}", device.vendor());
            LOGINFO("  Version: {}", device.version());
            LOGINFO("  Type: CPU={}, GPU={}, Accelerator={}", device.is_cpu(), device.is_gpu(), device.is_accelerator());
            ...
            try {
                ...
                LOGINFO("  Capabilities: FP64={}, Atomic64={}, USM(D/H/S)={}/{}/{}",
                        supports_fp64, supports_atomic64,
                        supports_usm_device, supports_usm_host, supports_usm_shared);
            } catch (const sycl::exception& e) {
