...
public:
    ...
    const Matrix3& basis() const { return basis_; }
    const Matrix3& basisInv() const { return basisInv_; }
    ...
    float* data() { return val_buffer_->data(); }
    const float* data() const { return val_buffer_->data(); }
...
HOST DEVICE Vector3_t<int> BaseGrid::applyBoundaryConditions(int i, int j, int k) const {
    ...
    if constexpr (BC == BoundaryCondition::periodic) {
        ...
        if (i < 0) result.x = -i;
        else if (i >= nx_) result.x = 2 * nx_ - 1 - i;
        
        if (j < 0) result.y = -j;
        else if (j >= ny_) result.y = 2 * ny_ - 1 - j;
        
        if (k < 0) result.z = -k;
        else if (k >= nz_) result.z = 2 * nz_ - 1 - k;
    }
