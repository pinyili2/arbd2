...
void* send_ignoring_children_impl(const Resource& location, T& obj, T* dest) {
    ...
    case Resource::SYCL:
        ...
        if (location.is_local()) {
            ...
            ARBD::throw_not_implemented("`_send_ignoring_children(...)` on non-local SYCL");
        }
#else
    ...
    case Resource::METAL:
        ...
        if (location.is_local()) {
            ...
            ARBD::throw_not_implemented("`_send_ignoring_children(...)` on non-local METAL");
        }
#else
    ...
    default:
        ARBD::throw_value_error("`_send_ignoring_children(...)` applied with unknown resource type");
    }
...
template<typename T>
void* construct_remote_impl(const Resource& location, void* args, size_t args_size) {
    switch (location.type) {
    case Resource::SYCL:
#ifdef USE_SYCL
        if (location.is_local()) {
            ...
            // Note: This is simplified - proper implementation would handle args properly
    ...
    case Resource::CUDA:
        ...
        ARBD::throw_not_implemented("CUDA construct_remote should be handled in Proxy.cu");
        break;
...
template void* send_ignoring_children_impl<float>(const Resource&, float&, float*);
template void* send_ignoring_children_impl<double>(const Resource&, double&, double*);
//template void* send_ignoring_children_impl<Vector3>(const Resource&, Vector3&, Vector3*);
...
//template void* construct_remote_impl<Vector3>(const Resource&, void*, size_t);
