...
TEST_CASE("Metal Vector Addition", "[metal][kernels]") {
	...
	try {
		...
		const size_t n = 1;  // Test with just 1 element first // Very small test size for debugging
		...
		for (size_t i = 0; i < n; ++i) {
			...
			REQUIRE(result[i] == 42.0f); // Debug constant
		}
